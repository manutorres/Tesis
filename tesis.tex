...
servidores, tanto el servidor del juego como el servidor de percepciones (presentado 
...
del juego, conteniendo la información asociada a la percepción del turno en disputa. 
...
(Knowledge). Cada uno de los datos de interés es representado mediante un término diferente. 
...
\begin{verbatim}    % En castellano para mantener el idioma y porque las b() están así.
...
predicado \texttt{b/1} (beliefs). Este tipo de creencias es empleado directamente por 
el módulo encargado de la toma de decisiones, y se mantienen vigentes sólo durante el 
...
mediante argumentos de un predicado especial, \texttt{h/1} (hypothetical).
...
considera que existen razones para creer realizables solo aquellos deseos que satisfacen 
...
optimal de acciones, que tiene en cuenta no solo el nodo destino, sino también los recursos 
...
la acción nula (skip) puede ser reemplazada por la acción de recargar energía, si es que 
...
Esta es una etapa de verificación, que tiene como objetivo la detección de este tipo de 
...
Entre las condiciones de corte tenidas en cuenta, se encuentran: que haya pasado una determinada 
cantidad de turnos desde el inicio del plan, que el agente se encuentre deshabilitado, que 
haya sido atacado o se encuentre amenazado por un enemigo, o que la meta haya sido alcanzada 
...
%REFFFFFFFFF seteo de creencias.
