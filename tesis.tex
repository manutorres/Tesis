...
en cada turno de manera previa. Todos los tipos de creencias son explicados en el %REFFFFFFFFFFFFFF
...
del agente está determinado por el conjunto de las \textit{beliefs}, o creencias. En este
capítulo, se definirán, clasificarán y listarán éstas.
...
Dada la gran cantidad y complejidad de creencias, será conveniente clasificarlas. 
Propondremos dos maneras distintas de clasificarlas en las secciones subsiguientes.

\section{Clasificación por Residencia} % revisar "residencia"

Una manera en la que las creencias pueden ser diferenciadas es su lugar de residencia, o
sea, dónde fueron asertadas o dónde son calculadas. Los dos programas en los cuales pueden
estar son \textit{Prolog} o \textit{DeLP}.

\subsection{Prolog}

En este programa se encuentran las creencias que provienen de la percepción, y son 
asertadas de manera literal. Otras se generan a partir de inferencias, que no necesitan 
demasiado cálculo. Si la computación que requieren es excesiva, son asertadas para evitar
gastos de tiempo innecesarios.


\subsection{DeLP}

En DeLP, se utilizan varios tipos de creencias, las cuales serán tanto asertadas 
previamente, como inferidas, o también prefijadas por los desarrolladores. 


\subsubsection{Crencias Generadas}
...
\section{Idea general} %cambiar nombre ej estrategia disñio
...
\begin{definicion}
	...
	%cuyo functor denota el nombre del deseo que se desea representar, 
	y tiene por lo menos un parámetro. Estos parámetros
	no deben estar instanciados.
\end{definicion}

%\begin{ejemplo}
%Considere proge de la figura tal... en este contexto: caaminar(X,Y) y lalla(Z) son deseos para el agente. 
%\end{ejemplo}

Note que el functor del literal de deseo connota.....
...
\begin{definicion}
	Un deseo es un modelo de deseo instanciado.
...
\begin{definicion}
Una {regla de deseo} es una regla rebatible o una regla estricta cuya cabeza tiene un modelo de deseo.
\end{definicion}
...
\subsection{Interfaz con Argumentación}
...
\subsection{Defensa Propia}

Cuando la salud de un agente es reducida a cero, debido a los ataques de agentes saboteadores 
enemigos, queda deshabilitado, perdiendo así la mayoría de sus capacidades y perjudicando
directamente al equipo. Por esta razón, los agentes deben tener la habilidad de reconocer
situaciones de peligro, y, de ser posible, superarlas. Con este objetivo, se cuenta con el 
deseo \textbf{Defensa Propia}.

Son utilizadas dos reglas para implementar el deseo, como se aprecia en la Fig. \ref{fig:deseoDefensaPropia}.
...
auxilio(Valor, Reparador) -<
	...
	valorAuxilio(Distancia, EnergiaRestante, MaximaSalud, Salud, Negativo, Valor).  	
\end{verbatim}
...
al comprador incrementar su \textit{salud máxima}, \textit{energía máxima}, \textit{rango de visión}, 
y \textit{fuerza}, respectivamente.
...
Los agentes saboteadores, como se dijo, tienen la capacidad de atacar a cualquier otro 
...
atacar(Valor, Agente) -<
    ...
    b(distancia(Posicion,[[attack, Agente]], Turnos, EnergiaRestante)),
	b(difPuntosSinMi(DifPuntos)),
    rol(Agente, Rol),
	puntosPorRol(atacar, Rol, PuntosRol),
	posicionSegura(PuntosSeguro),
	valorAtacar(Turnos, EnergiaRestante, DifPuntos, PuntosSeguro, Puntos, Valor).
\end{verbatim}
...
reparar(Valor, Agente) -<
    ...
    b(distancia(_Posicion,[[repair, Agente]], Turnos, EnergiaRestante)),
    role(Agente, Rol),
	puntosPorRol(reparar, Rol, PuntosRol),
    valorReparar(Turnos, EnergiaRestante, PuntosRol, Salud, MaximaSalud, PuntosDeshabilitado, Valor).
\end{verbatim}
